...
* *This configuration ensures smooth operation of the host system while the microservices cluster is fully active.*
...
## ⚙️ Infrastructure Configuration (Run on VMs)
...
3.  **Network Selection:** Ensure the Network Interface is set to **"Virtual network 'default' : NAT"**.
    * *Note:* This ensures the VMs are placed on the `192.168.122.x` subnet required for our Static IP configuration.
4.  **OS Installation:** Proceed with the standard Ubuntu installation.
    * *Tip:* Enable "Install OpenSSH Server" during setup to allow remote access immediately.

### **Step 1: Network Configuration (Static IP)**
*Run this on **EACH** VM, changing the IP address to `.10`, `.11`, or `.12` respectively.*
...
*Run these commands on **ALL THREE** VMs to install the Docker runtime.*
